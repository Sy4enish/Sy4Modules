...
class AIRouteUltimateMod(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            loader.ConfigValue("active_provider", "gemini",
                "Текущий провайдер (openai/anthropic/gemini/deepseek/qwen/openrouter/codex/nvidia)"),
            loader.ConfigValue("image_provider", "gemini",
                "Провайдер картинок (openai/gemini)"),
            
            loader.ConfigValue("openai_api_key", "", "API ключ OpenAI", validator=loader.validators.Hidden()),
            loader.ConfigValue("openai_image_api_key", "", "API ключ OpenAI для картинок (оставьте пустым если совпадает)", validator=loader.validators.Hidden()),
            ...
            loader.ConfigValue("anthropic_model", "claude-opus-4-7-20260416", "Модель Claude"),
            
            loader.ConfigValue("gemini_api_key", "", "API ключ Gemini (AIza...)", validator=loader.validators.Hidden()),
            loader.ConfigValue("gemini_image_api_key", "", "API ключ Gemini для картинок (оставьте пустым если совпадает)", validator=loader.validators.Hidden()),
            ...
            loader.ConfigValue("openrouter_model", "anthropic/claude-3.7-sonnet", "Модель OpenRouter"),
            
            loader.ConfigValue("codex_api_key", "", "API ключ Codex (OpenAI)", validator=loader.validators.Hidden()),
            loader.ConfigValue("codex_model", "gpt-4.5-preview", "Модель Codex"),
    ...
    async def _execute_tools(self, text, message=None):
        ...
        clean = re.sub(r"<TOOL:[^>]+>", "", text).strip()
    ...
    async def _run_agent_loop(self, provider, prompt, chat_id, message,
        ...
        hist = self._get_history(chat_id) if not override_sys else []
        if not override_sys:
            self._update_history(chat_id, "user", prompt)
        ...
        for step_num in range(1, max_steps + 1):
            ...
            res = await self._call_provider_api(provider, current_prompt, hist, sys_prompt, img, mime)

            if not override_sys:
                self._update_history(chat_id, "model", res)
            ...
            if step_logs:
                step_header = f"<b>— Шаг {step_num}</b>"
                formatted = "\n".join(f"  {l}" for l in step_logs)
                all_step_logs.append(f"{step_header}\n{formatted}")
            ...
            if not override_sys:
                self._update_history(chat_id, "user", current_prompt)
    ...
    async def _try_inline(self, message, text, buttons):
        """Пробуем инлайн, фолбек — обычный ответ"""
        try:
    ...
    async def _regen_cb(self, call: InlineCall, provider: str, chat_id: str, prompt: str):
        ...
        try:
            ...
            hist2 = self._get_history(chat_id)
            meta = f"🗜 [{len(hist2)}/{self.config['max_history_length']}] {model_name} | 🔄 рег."
            final_text = (
    ...
    async def _reset_cb(self, call: InlineCall, chat_id: str):
        ...
        model_name = self.config.get(f"{provider}_model", provider)
        max_len = self.config["max_history_length"]
        meta = f"🗜 [0/{max_len}] {model_name} | 🧹 память сброшена"
        try:
            await call.edit(
                f"<i>{utils.escape_html(meta)}</i>\n\n"
    ...
    async def _regen_img_cb(self, call: InlineCall, provider: str, prompt: str):
        ...
        try:
            if provider == "openai":
                key = self.config.get("openai_image_api_key") or self.config["openai_api_key"]
                img_bytes, err = await self._call_openai_image(prompt, key, self.config["openai_image_model"])
            elif provider == "gemini":
                key = self.config.get("gemini_image_api_key") or self.config["gemini_api_key"]
    ...
    async def _cfg_main_menu(self, call_or_msg, edit=True):
        ...
        if edit and hasattr(call_or_msg, "edit"):
            ...
            await self.inline.form(message=call_or_msg, text=text, reply_markup=btns)
    ...
    async def _cfg_provider_menu_cb(self, call: InlineCall, provider: str):
        ...
        if provider in ("openai", "gemini"):
            ...
            img_key = self.config.get(f"{provider}_image_api_key", "")
            img_key_display = (img_key[:6] + "..." + img_key[-4:]) if len(img_key) > 10 else ("задан" if img_key else "как основной")
            text += f"\n🖼 Модель картинок: <code>{img_model}</code>\n🔑 Ключ картинок: <code>{img_key_display}</code>"
        btns = [
            [
    ...
    async def _cfg_clear_key_cb(self, call: InlineCall, provider: str):
        ...
        if provider in ("openai", "gemini"):
            self.config[f"{provider}_image_api_key"] = ""
        await call.answer("🗑 Ключи удалены", show_alert=False)
    ...
    async def _collect_media(self, message, prompt):
        """Собирает медиа из реплая, возвращает (prompt, image_bytes, mime_type)"""
        image_bytes = None
        mime_type = None
        reply = await message.get_reply_message()
        if not reply:
        ...
        if getattr(reply, "photo", None):
            ...
            try:
                ...
                pass
        if not image_bytes and reply.sender_id:
            if any(w in prompt.lower() for w in ["ава", "аватар", "аву", "фото профил"]):
                try:
                    ...
                    pass
        if reply.text and not image_bytes:
            prompt += f"\n\nОтветь на это: {reply.text}"
    ...
    async def acmd(self, message: Message):
        ...
        try:
            ...
            model_name = self.config.get(f"{provider}_model", provider)
            hist = self._get_history(message.chat_id)
            meta = f"🗜 [{len(hist)}/{self.config['max_history_length']}] {model_name} | {elapsed}с"
            safe_prompt = utils.escape_html(prompt[:800]) + ("…" if len(prompt) > 800 else "")
            final_text = (
                f"<i>{utils.escape_html(meta)}</i>\n\n"
    ...
    async def aimgcmd(self, message: Message):
        ...
        if provider == "openai":
            key = self.config.get("openai_image_api_key") or self.config.get("openai_api_key")
            if not key: return await utils.answer(status, "укажите openai_image_api_key")
            img_bytes, err = await self._call_openai_image(prompt, key, self.config["openai_image_model"])
        elif provider == "gemini":
            key = self.config.get("gemini_image_api_key") or self.config.get("gemini_api_key")
            if not key: return await utils.answer(status, "укажите gemini_image_api_key")
            img_bytes, err = await self._call_gemini_image(prompt, key)
        elif provider == "gemini_hub":
            key = self.config.get("gemini_hub_api_key")
        ...
        try:
            await self.inline.form(
                message=status,
                text="🖼 <b>Действия с картинкой:</b>",
                reply_markup=self._img_buttons(provider, prompt),
            )
        except Exception:
            await status.delete()

    async def _web_search(self, query: str, max_results: int = 5) -> list:
        """Поиск через SearXNG → DDG → worldtimeapi (для времени). Возвращает список {title, snippet, url}"""
        results = []
        headers = {
        ...
        # 1) SearXNG публичный инстанс (JSON, без ключа)
        searx_instances = [
            "https://searx.be",
                ...
                async with aiohttp.ClientSession() as s:
                    async with s.get(
                        ...
                        if r.status == 200:
                            ...
                            snippets = re.findall(r'class="result__snippet"[^>]*>(.*?)</(?:span|div|a)>', html, re.DOTALL)
        ...
        # fallback: worldtimeapi для запросов о времени
        if not results:
            q = query.lower()
    ...
    async def asearchcmd(self, message: Message):
        ...
        try:
            ...
            if sources_text:
                final_text += f"\n\n📎 <b>Источники:</b><blockquote>{utils.escape_html(sources_text[:600])}</blockquote>"
            ...
            try:
                await self.inline.form(message=status, text=final_text, reply_markup=btns)
            except Exception:
    ...
    async def _search_regen_cb(self, call: InlineCall, provider: str, query: str):
        ...
        try:
            ...
            if sources_text:
                final_text += f"\n\n📎 <b>Источники:</b><blockquote>{utils.escape_html(sources_text[:400])}</blockquote>"
            btns = [[
                {"text": "🔄 Переспросить", "callback": self._search_regen_cb,
                 "args": (provider, query)},
                {"text": "❌ Закрыть", "callback": self._close_menu_cb, "args": ()},
    ...
    async def aprovidercmd(self, message: Message):
        ...
        try:
            await self.inline.form(
                message=message,
                text=f"<b>Выберите провайдера:</b>\nТекущий: <code>{current}</code>",
                reply_markup=btns,
            )
        except Exception:
            await utils.answer(message, f"<b>Провайдеры:</b> <code>{', '.join(valid)}</code>")
    ...
    async def aimgauthcmd(self, message: Message):
        ...
        try:
            await self.inline.form(
                message=message,
                text=f"<b>Провайдер картинок:</b> <code>{current}</code>",
                reply_markup=btns,
            )
        except Exception:
            await utils.answer(message, f"<b>Доступно:</b> <code>{', '.join(valid)}</code>")
    ...
    async def amodelcmd(self, message: Message):
        ...
        try:
                ...
                if provider == "openai":
                    ...
                    async with aiohttp.ClientSession() as s:
                        async with s.get("https://api.openai.com/v1/models",
                            ...
                            data = await r.json()
                            # фильтруем только нужные нам — GPT и o-серия
                            models = sorted([
                    ...
                    async with aiohttp.ClientSession() as s:
                        async with s.get("https://openrouter.ai/api/v1/models",
                            ...
                            data = await r.json()
                            # берём только бесплатные или топ-50
            ...
            await self.inline.form(
                message=status,
                text=(f"<tg-emoji emoji-id=5985780596268339498>🤖</tg-emoji> "
    ...
    async def aagentcmd(self, message: Message):
        ...
        try:
            ...
            self.config["agent_mode"] = not agent
            state = "включён" if self.config["agent_mode"] else "выключен"
            await utils.answer(message, f"🤖 <b>Агент {state}</b>")
    ...
    async def astatcmd(self, message: Message):
        ...
        text = (
            ...
            f"🗜 <b>История:</b> {total_msgs} сообщ. в {chat_count} чатах\n\n"
            f"<b>Ключи:</b>\n{keys_line}"
        )
        btns = [
            [
    ...
    async def askillcmd(self, message: Message):
        ...
        if not args or args == "list":
            ...
            btns.append([{"text": "❌ Закрыть", "callback": self._close_menu_cb, "args": ()}])
            try:
                return await self.inline.form(message=message, text=text, reply_markup=btns)
            except Exception:
    ...
    async def ahelpcmd(self, message: Message):
        ...
        text = (
            ...
            "<b>Управление:</b>\n"
            "• <code>.aprovider [name]</code> — сменить провайдер\n"
